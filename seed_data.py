...
def seed_database():
    """Seeds the database with users, extensive food items, and goals."""
    ...
    try:
        ...
        cursor.executemany('''
            INSERT OR IGNORE INTO Users (user_id, name, email, age, gender, height, weight, daily_calorie_goal)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', users_data)

        # --- 2. SEED NUTRITIONAL GOALS (For Half the Users: IDs 1-5) ---
        ...
        cursor.executemany('''
            INSERT OR IGNORE INTO NutritionGoals (goal_id, user_id, daily_calories, protein_goal, carbohydrate_goal, fat_goal, fiber_goal)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', goals_data)

        # --- 3. SEED 80 FOOD ITEMS ---
        ...
        cursor.executemany('''
            INSERT OR IGNORE INTO Foods (food_name, serving_size, calories, protein, carbohydrates, fat, fiber, sugar)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', foods_data)

        conn.commit()
        print(f"Success! Inserted {len(users_data)} users, {len(goals_data)} goals, and {len(foods_data)} food items.")
